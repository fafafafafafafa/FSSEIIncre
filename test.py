...
def test_one_epoch(support, query, support_labels, query_labels, encoder):
    """
    单次epoch训练
    参数说明:
        输入:
...
def test():
    ...
    # test_data_loader = torch.plot_utils.data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False)

    if args.model_name == 'cvcnn1':
        encoder = MyModel.base_model(args, mode=False)
    else:
    ...
    N_ways = args.test_dataset_num
    K_shots = args.test_dataset_k_shot
    N_query = args.test_dataset_query
    ...
    for N_way in N_ways:
        ...
        for K_shot in K_shots:
            ...
            for epoch in range(epochs):
                ...
                print('Epoch {}/{} {}way {}shot'.format(epoch, epochs, N_way, K_shot))
                # 随机抽取 N 类
                ...
                test_loss, acc = test_one_epoch(support, query, support_labels, query_labels, encoder)
            ...
            test_data_dir_path = args.data_dir_path + args.model_name + '/' + args.loss_fn_name + '/' + \
