...
class Inc_Learning_Appr:
        ...
        if self.warmup_epochs and t > 0:
            ...
            for e in range(self.warmup_epochs):
                ...
                for images, targets in trn_loader:
                    ...
                    loss = self.warmup_loss(outputs[t], targets.long().to(self.device) - self.model.task_offset[t])
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        for e in range(self.nepochs):
            ...
            if self.eval_on_train:
                train_loss, train_taw_acc, train_tag_acc, train_tag_acc_topk = self.eval(t, trn_loader)
                clock2 = time.time()
                print('| Epoch {:3d}, time={:5.1f}s/{:5.1f}s | Train: loss={:.3f}, '
                      'TAw acc={:5.1f}%, Tag acc={:5.1f}% |'
                      'Tag topk{} acc={:5.1f}% |'.format(e + 1, clock1 - clock0, clock2 - clock1, train_loss,
                                                         100 * train_taw_acc, 100 * train_tag_acc,
                                                         self.topk, 100 * train_tag_acc_topk),end='')

                self.logger.log_scalar(task=t, iter=e + 1, name="loss", value=train_loss, group="train")
                self.logger.log_scalar(task=t, iter=e + 1, name="taw_acc", value=100 * train_taw_acc, group="train")
                self.logger.log_scalar(task=t, iter=e + 1, name="tag_acc", value=100 * train_tag_acc, group="train")
                self.logger.log_scalar(task=t, iter=e + 1, name="tag_acc_topk{}".format(self.topk),
                                       value=100 * train_tag_acc_topk, group="train")
            else:
                print('| Epoch {:3d}, time={:5.1f}s | Train: skip eval |'.format(e + 1, clock1 - clock0), end='')

            # Valid
            clock3 = time.time()
            valid_loss, valid_taw_acc, valid_tag_acc, valid_tag_acc_topk = self.eval(t, val_loader)
            clock4 = time.time()
            print(' Valid: time={:5.1f}s loss={:.3f}, TAw acc={:5.1f}%, Tag acc={:5.1f}%, Tag topk{} acc={:5.1f}% |'.format(
                clock4 - clock3, valid_loss, 100 * valid_taw_acc, 100 * valid_tag_acc, self.topk, 100 * valid_tag_acc_topk), end='')
            self.logger.log_scalar(task=t, iter=e + 1, name="loss", value=valid_loss, group="valid")
            self.logger.log_scalar(task=t, iter=e + 1, name="taw_acc", value=100 * valid_taw_acc, group="valid")
            self.logger.log_scalar(task=t, iter=e + 1, name="tag_acc", value=100 * valid_tag_acc, group="valid")
            self.logger.log_scalar(task=t, iter=e + 1, name="tag_acc_topk{}".format(self.topk),
                                   value=100 * valid_tag_acc_topk, group="valid")
    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images, targets in trn_loader:
            ...
            outputs = self.model(images.to(self.device))
            # print('outputs', outputs[t].shape)
            # print('targets', targets.shape)
            loss = self.criterion(t, outputs, targets.long().to(self.device))
            # Backward
            self.optimizer.zero_grad()
            loss.backward()
    ...
    def eval(self, t, val_loader):
        ...
        with torch.no_grad():
            ...
            for images, targets in val_loader:
                ...
                targets = targets
                loss = self.criterion(t, outputs, targets.long().to(self.device))
                hits_taw, hits_tag, hits_tag_topk = self.calculate_metrics(outputs, targets)
                # Log
                total_loss += loss.item() * len(targets)
                total_acc_taw += hits_taw.sum().item()
                total_acc_tag += hits_tag.sum().item()
                total_acc_tag_topk += hits_tag_topk.sum().item()
                total_num += len(targets)
        return total_loss / total_num, total_acc_taw / total_num,\
            total_acc_tag / total_num, total_acc_tag_topk / total_num
    ...
    def calculate_metrics(self, outputs, targets):
        ...
        # topk  [bs, topk]
        pred_topk = torch.topk(outputs, k=self.topk, dim=1, largest=True,sorted=True)[1]
        # print(pred)
        hits_tag = (pred_topk[:, 0] == targets.to(self.device)).float()
        hits_tag_topk = (pred_topk == torch.unsqueeze(targets, 1).to(self.device)).float()
        return hits_taw, hits_tag, hits_tag_topk
