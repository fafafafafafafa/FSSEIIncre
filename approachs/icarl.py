...
class Appr(Inc_Learning_Appr):
    ...
    def classify(self, task, features, targets):
        ...
        means = torch.stack(self.exemplar_means)    # [K, feat_dim]
        means = torch.stack([means] * features.shape[0])    # [batch_size, K, feat_dim]
        means = means.transpose(1, 2)   # # [batch_size, feat_dim, K]
        # expand all features to all classes
        features = features / features.norm(dim=1).view(-1, 1)  # [batch_size, feat_dim]
        features = features.unsqueeze(2)    # [batch_size, feat_dim, 1]
        features = features.expand_as(means)    # [batch_size, feat_dim, K]
        # get distances for all images to all exemplar class means -- nearest prototype
        dists = (features - means).pow(2).sum(1).squeeze()  # [batch_size, K]
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        for e in range(self.nepochs):
            ...
            if self.eval_on_train:
                train_loss, train_taw_acc, train_tag_acc = self.eval(t, trn_loader)
                clock2 = time.time()
                print('| Epoch {:3d}, time={:5.1f}s/{:5.1f}s | Train: loss={:.3f}, '
                      'TAw acc={:5.1f}%, Tag acc={:5.1f}% |'.format(e + 1, clock1 - clock0, clock2 - clock1, train_loss,
                                                                    100 * train_taw_acc, 100 * train_tag_acc), end='')
            ...
            # Valid
            clock3 = time.time()
            valid_loss, valid_taw_acc, valid_tag_acc = self.eval(t, val_loader)
            clock4 = time.time()
            print(' Valid: time={:5.1f}s loss={:.3f}, TAw acc={:5.1f}%, Tag acc={:5.1f}% |'.format(
                clock4 - clock3, valid_loss, 100 * valid_taw_acc, 100 * valid_tag_acc), end='')
        ...
        self.exemplars_dataset.collect_exemplars(self.model, trn_loader, None)

        # compute mean of exemplars
        self.compute_mean_of_exemplars(trn_loader, None)
