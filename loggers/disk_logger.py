...
class Logger(ExperimentLogger):
    ...
    def __init__(self, log_path, exp_name, begin_time=None):
        ...
        sys.stdout = FileOutputDuplicator(sys.stdout,
                                          os.path.join(self.exp_path, 'stdout.txt'),
                                          'w')
        sys.stderr = FileOutputDuplicator(sys.stderr,
                                          os.path.join(self.exp_path, 'stderr.txt'),
                                          'w')
    ...
    def log_result(self, array, num_tasks, name, step):
        if array.ndim <= 1:
            array = array[None]
        np.savetxt(os.path.join(self.exp_path, 'results', '{}tasks'.format(num_tasks),'{}.txt'.format(name)),
                   array, '%.6f', delimiter='\t')

    def log_figure(self, name, iter, num_tasks, figure, curtime=None):
        curtime = datetime.now()
        figure.savefig(os.path.join(self.exp_path, 'figures', "{}tasks".format(num_tasks),
                                    '{}.png'.format(name)))
        plt.close()
        '''
        figure.savefig(os.path.join(self.exp_path, 'figures', "{}tasks".format(num_tasks),
    ...
    def save_model(self, state_dict, num_tasks, task):
        torch.save(state_dict, os.path.join(self.exp_path, "models", "{}tasks".format(num_tasks), "task{}.ckpt".format(task)))
