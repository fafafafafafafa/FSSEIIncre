...
def to_one_hot(labels: torch.Tensor, num_classes: int, dtype: torch.dtype = torch.float, dim: int = 1) -> torch.Tensor:
    ...
    # o.shape = (B, N, H, W) labels.shape = (B, 1, H, W)
...
class FocalLoss(_Loss):
    ...
    def __init__(self,
        ...
        # gamma == 0 时, 退化为 CrossEntropyLoss
        self.gamma = gamma
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
       ...
       """
        ...
        # print(self.epsilon)
        focal_loss = (1 - pt).pow(self.gamma) * self.ce_loss

        if self.reduction == 'mean':
            focall = torch.mean(focal_loss)  # the batch and channel average
        elif self.reduction == 'sum':
            focall = torch.sum(focal_loss)  # sum over the batch and channel dims
