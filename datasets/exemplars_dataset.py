...
class ExemplarsDataset(MemoryDataset):
    ...
    def __init__(self, transform, class_indices,
        ...
        super().__init__({'x': [], 'y': []}, class_indices=class_indices, transform_x=transform)
    ...
    def extra_parser(args):
        ...
        # 创建一个互斥组，确保只有一个参数在命令行中可用 num-exemplars和num-exemplars-per-class只能使用其中一个
        ...
        parser.add_argument('--exemplar-selection', default='herding', type=str,
    ...
    def collect_exemplars(self, model, trn_loader, selection_transform):
        if self._is_active():
            self.X, self.Y = self.exemplars_selector(model, trn_loader, selection_transform)
