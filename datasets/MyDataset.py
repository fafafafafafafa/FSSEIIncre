...
class MyFsSeiDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, filepath_x, filepath_y, class_indices=None, transform_x=None, transform_y=None):
...
def get_test_dataset(num, transform_x=None, transform_y=None):
    filepath_x = '../SplitFSSEIDataset/X_test_{}Class.npy'.format(num)
    filepath_y = '../SplitFSSEIDataset/Y_test_{}Class.npy'.format(num)

    return MyFsSeiDataset(filepath_x, filepath_y, transform_x, transform_y)


def get_test_support_dataset(num, transform_x=None, transform_y=None):
    x = np.load('../SplitFSSEIDataset/X_train_{}Class.npy'.format(num))
    y = np.load('../SplitFSSEIDataset/Y_train_{}Class.npy'.format(num))
    if transform_x:
        x = transform_x(x)
    if transform_y:
        y = transform_y(y)
